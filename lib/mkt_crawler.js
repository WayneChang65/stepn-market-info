...
let stopSelector_beforeSearch =
	'ul.flex-sp-block > li:nth-child(3) > a:nth-child(1)';
...
async function _getShoesQuantity(options) {
	...
	try {
		...
		let valFloor = await page.evaluate(() => {
			const hitsSelector =
				'.table > tbody:nth-child(2) > tr:nth-child(1) > td:nth-child(2)';
			let nlHits = document.querySelectorAll(hitsSelector);
			let aryHits = Array.from(nlHits).map((xxx) => xxx.innerText);
			return parseFloat(aryHits[0].split(' ')[0]);
		});

		result = {
			type: shoesType,
			quality: parseInt(hits[0].split(' ')[0].replace(',', '')),
			floor: valFloor,
		};
	} catch (e) {
		console.log(e);
