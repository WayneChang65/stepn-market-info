...
function _getCurrentDateTime(_shiftInDays) {
	...
	// 	gDt.offsetInHours(8);
	// }
	dateTime.setShortWeekNames(['日', '一', '二', '三', '四', '五', '六']);
...
function _checkNewItemFromArray(oldArray, newArray) {
	...
	oldArray.forEach((item) => {
		if (
			!typeof item == 'object' ||
			!_checkOwnProperty(item, 'title') ||
			!_checkOwnProperty(item, 'content') ||
			!_checkOwnProperty(item, 'href') ||
			!_checkOwnProperty(item, 'num')
		)
	...
	oldArray.forEach((oldItem) => {
		tempNewArray = tempNewArray.filter(
